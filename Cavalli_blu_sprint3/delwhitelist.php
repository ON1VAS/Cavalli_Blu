<html>
 <head><title>Cancella Whitelist</title></head>
  <body>
<?php
session_start();
...
if ($admin == 1) {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		...
		$check = "SELECT * FROM whitelist WHERE sito = '$sito'";
		$ris = mysqli_query($conn, $check);
		if(mysqli_num_rows($ris) == 0){
			echo "<h1>Il sito non è presente nella whitelist! </h1><br>";
            echo '<script>
                function goBack() {
			...
			$sql = "DELETE FROM whitelist WHERE sito = '$sito'";
			$result = mysqli_query($conn, $sql);
			if ($result) {
				echo "<h1>Sito rimosso dalla whitelist! </h1><br/>";
				echo '<script>
					function goBack() {
					window.location.href = document.referrer;
				}
		...
		// nello script usiamo window.history.back() invece di document.deferrer
		// history.back => pagina precedente nella cronologia del browser
		// document.deferrer => restituisce l'url della pagina precedente visitata dall'utente
		// location.href => se vogliamo puntare una pagina specifica
			echo '<!DOCTYPE html> 
      <html> 
        <script>
                function goBack() {
          ...
          <body class="align"> 
            ...
            <h1 style="color: var(--baseColor); margin-bottom: 4px;">Rimuovi sito Whitelist</h1> 
                      <div class="form__field"> 
                          <label style="color: var(--iconFill); font-weight: bolder;">Word</label> 
                                              
                          <input autocomplete="link" type="text" name="Link" class="form__input" placeholder="Inserisci un ID valido" required=""> 
                      </div>   
