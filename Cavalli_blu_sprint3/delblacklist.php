<html>
 <head><title>Cancella Blacklist</title></head>
  <body>
<?php
session_start();
...
if ($admin == 1) {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		...
		if(mysqli_num_rows($ris) == 0){
			echo "<h1>Il sito non è presente nella blacklist! </h1><br/>";
            echo '<script>
                function goBack() {
			...
			$sql = "DELETE FROM blacklist WHERE sito = '$sito'";
			$result = mysqli_query($conn, $sql);
			if ($result) {
				echo "<h1>Sito rimosso dalla blacklist! </h1><br/>";
				echo '<script>
					function goBack() {
					window.location.href = document.referrer;
				}
      ...
      <html> 
        <script>
          ...
          <body class="align"> 
            ...
            <h1 style="color: var(--baseColor); margin-bottom: 4px;">Rimuovi sito Blacklist</h1> 
