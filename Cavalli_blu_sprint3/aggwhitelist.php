<html>
 <head><title>Aggiungi Whitelist</title></head>
  <body>
<?php
session_start();
...
if ($admin == 1) {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		...
		if(mysqli_num_rows($ris) > 0){
			echo "<p>Il sito è già presente nella whitelist! </p><br/>";
            echo '<script>
                function goBack() {
			...
			$sql = "INSERT INTO whitelist (sito) VALUES ('$sito')";
			$result = mysqli_query($conn, $sql);
			if ($result) {
				echo "<h1>Sito inserito nella whitelist! </h1><br/>";
				echo '<script>
					function goBack() {
					window.location.href = document.referrer;
				}
      ...
      <html> 
        <script>
          ...
          <body class="align"> 
            ...
            <h1 style="color: var(--baseColor); margin-bottom: 4px;">Aggiungi sito Whitelist</h1> 
